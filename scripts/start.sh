...
prop_replace 'nifi.web.http.host'               "${NIFI_WEB_HTTP_HOST:-$fqdn}"
prop_replace 'nifi.remote.input.host'           "${NIFI_REMOTE_INPUT_HOST:-$fqdn}"
prop_replace 'nifi.remote.input.socket.port'    "${NIFI_REMOTE_INPUT_SOCKET_PORT:-10000}"
prop_replace 'nifi.remote.input.secure'         'false'

# Set nifi-toolkit properties files and baseUrl
"${scripts_dir}/toolkit.sh"
prop_replace 'baseUrl' "http://${NIFI_WEB_HTTP_HOST:-$fqdn}:${NIFI_WEB_HTTP_PORT:-8080}" ${nifi_toolkit_props_file}
...
prop_replace 'nifi.cluster.node.address'                    "${NIFI_CLUSTER_ADDRESS:-$fqdn}"
...
prop_replace 'nifi.ui.bannner.text'     "${NIFI_BANNER_TEXT}:-"
